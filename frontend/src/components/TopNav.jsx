...
export function TopNav({ currentPage, onNavigate }) {
  ...
  const handleNavigate = id => { onNavigate(id); setIsMenuOpen(false) }
  return <nav className="topnav">
    <button onClick={()=>handleNavigate('dashboard')} className="topnav-logo" aria-label="Countor home"><img src={logoImg} alt="Countor" /></button>
    <button className="mobile-menu-btn" onClick={()=>setIsMenuOpen(!isMenuOpen)} aria-label="Toggle navigation" aria-expanded={isMenuOpen}>{isMenuOpen?'✖':'☰'}</button>
    <div className={`topnav-links ${isMenuOpen?'open':''}`}>
      {LINKS.filter(l=>l.show).map(link=><button key={link.id} onClick={()=>handleNavigate(link.id)} className={`topnav-link ${currentPage===link.id?'active':''}`}>{link.label}</button>)}
