...
async function req(method, path, body = null) {
  ...
  if (contentType.includes('text/csv')) { if (!res.ok) throw new Error('Export failed'); return res.blob() }
...
export const communityAPI = {
  posts: (category, sort) => http.get(`/api/posts?category=${category || 'all'}&sort=${sort || 'new'}`), createPost: body => http.post('/api/posts', body), deletePost: id => http.delete(`/api/posts/${id}`), upvotePost: id => http.post(`/api/posts/${id}/upvote`),
...
export const adminAPI = {
  ...
  exportCSV: async (orgId, userId) => {
    ...
    const blob = await req('GET', `/api/admin/export?${params}`); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = orgId ? 'countor_org.csv' : userId ? 'countor_user.csv' : 'countor_all.csv'; a.click(); URL.revokeObjectURL(url)
  },
}
...
export async function openReferral(company, tier, source = 'results') {
  ...
  try { const { url } = await referralAPI.click(company.id, company.name, tier, source, destUrl); window.open(url, '_blank', 'noopener,noreferrer') } catch { window.open(destUrl, '_blank', 'noopener,noreferrer') }
}
