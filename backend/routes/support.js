  ...
  try {
    ...
    const { rows } = await pool.query(`
      SELECT r.id, r.status, r.reason, r.created_at, r.updated_at,
             COALESCE(json_agg(json_build_object('id',m.id,'senderRole',m.sender_role,'message',m.message,'createdAt',m.created_at) ORDER BY m.created_at) FILTER (WHERE m.id IS NOT NULL), '[]') AS messages
...
router.post('/', requireUser, async (req, res) => {
  try {
    ...
    res.status(201).json({ ...rows[0], memberCode })
  } catch (err) {
    console.error(err)
