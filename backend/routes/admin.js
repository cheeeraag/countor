...
router.get('/stats', requireAdmin, async (req, res) => {
  try {
    const orgFilter = req.user.role === 'org_admin' ? `AND u.org_id = '${req.user.orgId}'` : ''
...
router.get('/users', requireAdmin, async (req, res) => {
  ...
  const params    = req.user.role === 'org_admin' ? [req.user.orgId] : []
...
router.get('/export', requireAdmin, async (req, res) => {
  ...
  const effectiveOrgId = req.user.role === 'org_admin' ? req.user.orgId : orgId
